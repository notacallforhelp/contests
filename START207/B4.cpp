...
void solve()
{
    ...
    {
        bool cont = false;
        if(s.count({-A[i],i})) cont=true;

        if(cont) s.erase(s.lower_bound({-A[i],i}));
        ...
        if(s.empty())
        {
            ...
            for(int i=0;i<n;i++)
            {
                if(i==0)
                {
                    P[i+1]=prs[n-1].second;
                }
                else
                {
                    P[i+1]=prs[i-1].second;
                }

                if(A[P[i+1]]<=A[i+1]) ++score;
            }
        ...
        if(cont) s.insert({-A[i],i});

        if(P[i]==i) invalid = true;
        if(A[P[i]]<=A[i]) ++score;
    }

    cout << (invalid?-1:score) << endl;
