...
{   
   ...
   if(g>=r&&g>=b) output.push_back('G'), --M['G'];
   else if(r>=b&&r>=g) output.push_back('R'), --M['R'];
   else if(b>=r&&b>=g) output.push_back('B'), --M['B'];
   ...
   while(true)
   {
        ...
        if(M['R']>0) C.insert('R');
        if(M['B']>0) C.insert('B');
        if(C.count(output[idx])) C.erase(output[idx]);
        if(idx-2>=0&&C.count(output[idx-2])) C.erase(output[idx-2]);
        ...
        else if(C.size()==2)
        {
            ...
            while(!C.empty())
            {
                p.push_back(*C.begin()); C.erase(C.begin());
            }
