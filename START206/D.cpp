...
void solve()
{
    ...
    {
        int c1 = (1ll<<i);
        if(c1==N) output = min(output,i);
        if(c1>=N) break;

        if((N-c1)%D!=0) continue;

        int k = (N-c1)/D;

        //int msb = 64-__builtin_clzll(k)-1;
        int c2 = i;  //allowed left shifts
        ...
        for(int bit=0;bit<=60;bit++)
        {
            if((1ll<<bit)&k)
            {
                if(bit<=i)
                {
                    ++c2; //setting a one within allowed left shifted
                }
                else
                {
                    c2 += (1ll<<(bit-i)); //repeated addition for out of bound bits
                }
            }
        }
        output = min(output,c2);
    }

    cout << (output==2e18?-1:output)<< endl;
}
