...
int func(string s, char c)
{
    ...
    for(int i=0;i<n;i++)
    {
        if(s[i]==c)
        {
            ...
            int rightwt = rightsum - cnt_right*i - cnt_right*(cnt_right+1)/2;
            int leftwt = i*cnt_left - leftsum - cnt_left*(cnt_left+1)/2;

            output = min(output,leftwt+rightwt);
