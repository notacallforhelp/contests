...
int center(string &s,char c)
{
    ...
    if(n%2!=0)
    {
        ...
        if(othermididx!=-1)
        {
            int lwr = 1; int output2 = 0;

            for(int i=othermididx-1;i>=0;i--)
            {
                if(s[i]==c)
                {
                    output2 += (othermididx-i)-lwr;
                    ++lwr;
                }
            }
            
            lwr = 1;
            for(int i=othermididx+1;i<n;i++)
            {
                if(s[i]==c)
                {
                    output2 += (i-mididx)-lwr;
                    ++lwr;
                }
            }
            output = min(output,output2);
        }
...
int left(string &s,char c)
{
    ...
    for(int i=0;i<n;i++)
    {
        ...
        while(leftend+1<n&&s[leftend+1]==c)
        {
            ++leftend;
        }
        if(seen) break;
    ...
    int lower = 1; int output = 0;

    for(int i=leftend+1;i<n;i++)
    {
        if(s[i]==c)
        {
            output += i-leftend-lower;
            ++lower;
        }
    }
...
int right(string &s,char c)
{
    ...
    for(int i=n-1;i>=0;i--)
    {
        if(s[i]==c) 
        {
            seen = true; rightend = i;
        }
        while(rightend-1>=0&&s[rightend-1]==c)
        {
            --rightend;
        }
        if(seen) break;
...
void solve()
{
    ...
    int output = min(center(s,'a'),center(s,'b'));
    output = min(output,left(s,'a'));
    output = min(output,left(s,'b'));
    output = min(output,right(s,'a'));
    output = min(output,right(s,'b'));
