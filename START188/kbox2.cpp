...
void solve()
{
    ...
    for(auto &ele:prs)
    {
        storage.insert({ele.second,ele.first});
    }
    ...
    while(j<k)
    {
        sum += storage.rbegin()->first;
        temp.insert(*storage.rbegin());
        auto last = prev(storage.end());
        storage.erase(last);
        ++j;
    }
    ...
    for(int i=0;i<n;i++)
    {
        if(!storage.empty()&&storage.count({prs[i].second,prs[i].first}))
        {
            storage.erase({prs[i].second,prs[i].first});
        }
        else if(!temp.empty()&&temp.count({prs[i].second,prs[i].first}))
        {
            sum -= prs[i].second;
            temp.erase({prs[i].second,prs[i].first});
            if(!storage.empty())
            {
                sum += storage.rbegin()->first;
                temp.insert(*storage.rbegin());
