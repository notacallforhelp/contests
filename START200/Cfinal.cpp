...
vector<vector<int>> Prob(N,vector<int>(2,0));
...
void solve()
{
    ...
    for(int i=n-1;i>=0;i--)
    {
        dp[i][0] = (Prob[A[i]][0]*dp[i+1][0] + Prob[A[i]][1]*dp[i+1][1])%mod;
        dp[i][1] = (Prob[A[i]][0]*dp[i+1][1] + Prob[A[i]][1]*dp[i+1][0])%mod;
    }
...
int32_t main()
{
    ...
    for(int i=1;i<=1000;i++)
    {
        ...
        {
            int left = i-j;
            Prob[i][0] = (Prob[i][0] + Prob[left][1])%mod;
            Prob[i][1] = (Prob[i][1] + Prob[left][0])%mod;
        }
        Prob[i][0] = (Prob[i][0]*pw(i,mod-2))%mod;
        Prob[i][1] = (Prob[i][1]*pw(i,mod-2))%mod;
    }
