...
void solve()
{
    ...
    while(n)
    {
        ...
        deals += j;
        M[power] += j;
        //output += j*(val/3)*(9+power);
        output += j*(val*3+power*(val)/3);
    ...
    for(int i=19;i>=1;i--)
    {
        ...
        /*for(int j=1;j<=M[i];j++)
        {
            if(deals-j+3*j<=k) accept = j;
        }*/

        deals = deals + 2*accept; // (deals - accept + 3*accept)

        output = (output - accept*f[i]+3*accept*f[i-1]);

        M[i] = M[i]-accept;
        M[i-1] = M[i-1] + 3*accept;
    }
