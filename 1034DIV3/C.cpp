...
void solve()
{
    ...
    vector<int> mx_suffix(n);
    vector<int> mn_prefix(n);

    mx_suffix[n-1]=A[n-1];
    
    for(int i=n-2;i>=0;i--)
    {
        mx_suffix[i] = max(mx_suffix[i+1],A[i]);
    }
    ...
    for(int i=1;i<n;i++)
    {
        mn_prefix[i]=min(mn_prefix[i-1],A[i]);
    }
    ...
    {
        bool good = false;
        if(mn_prefix[i]==A[i]&&mx_suffix[i]>=A[i])
        {
            good=true;
        }
        if(mx_suffix[i]==A[i]&&mn_prefix[i]<=A[i])
        {
            good=true;
        }
