    ...
    */
    ...
    while(T--)
    {
        ...
        for(auto &x:a)
            cin>>x.X>>x.Y;
        sort(all(a));
        multiset<lli> currentJobsEndTime;
     
        lli mnIdx = -1,mnVal = INF;
        lli mxIdx = -1,mxVal = -INF;
     
        for(lli day=1,idx=0;day+d-1<=n;day++){
            while(idx<k&&a[idx].X<=day+d-1){
                currentJobsEndTime.insert(a[idx].Y);
                idx++;
            }
            while(!currentJobsEndTime.empty()&&*currentJobsEndTime.begin()<day)
                currentJobsEndTime.erase(currentJobsEndTime.begin());
     
            const lli currentOverlap=sz(currentJobsEndTime);
