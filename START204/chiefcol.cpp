...
{
    int n,k; cin>>n>>k;
    vector<int> W(n); for(auto &ele:W)cin>>ele;
    vector<int> C(n); for(auto &ele:C)cin>>ele;

    vector<int> consec(n);
    vector<int> consec_val(n);
    consec[n-1]=1;
    consec_val[n-1]=W[n-1];

    for(int i=n-2;i>=1;i--)
    {
        if(C[i]==C[i+1])
        {
            consec_val[i]=W[i]+consec[i+1];
    ...
    for(int i=1;i<n;i++)
    {
        ...
        int op1 = -W[i];
        //int op2 = consec[i]*W[i]-k;
        int op2 = consec_val[i]-k;
        if(op2>=op1)
        {
            currcolor = C[i];
            money += W[i]-k;
