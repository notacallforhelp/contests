...
{   
   ...
   string s,t; cin>>s>>t;
    ...
    vector<vector<int>> next_occ(26,vector<int>(n+1,1e9));
    ...
    for(int i=0;i<26;i++)
    {
        ...
        for(int idx=n-1;idx>=0;idx--)
        {
            ...
            {
                next_occ[i][idx]=next_occ[i][idx+1];
            }
        }
    }
    ...
    for(int i=0;i<n;i++)
    {
        ...
        int t_ptr = 0;
        vector<int> tmp;
        while(s_ptr<n&&t_ptr<m&&s[s_ptr]==t[t_ptr])
        {
            tmp.push_back(s_ptr);
            ++t_ptr;
            if(t_ptr==m) break;
            int k = t[t_ptr]-'a';
            if(t[t_ptr]==s[s_ptr]) ++s_ptr;
            if(s_ptr>=n) break;
            s_ptr = next_occ[k][s_ptr];
        }
