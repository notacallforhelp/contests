...
{   
   ...
   vector<vector<int>> cur(m+1,vector<int>(2,INF));
   vector<vector<int>> future(m+1,vector<int>(2,INF));
   ...
   for(int i=0;i<n;i++)
   {
        ...
        for(int j=0;j<=m;j++)
        {
            for(int x=0;x<=1;x++)
            {
                future[j][0] = min(future[j][0],cur[j][x]+(x==1?1:0));

                if(j<m&&s[i]==t[j])
                {
                    future[j+1][1] = min(future[j+1][1],cur[j][x]);
                }
            }
