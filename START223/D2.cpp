   ...
   for(int i=0;i<n;i++)
   {
        ...
        for(auto &ele:owners[k])
        {
            ...
            for(auto &idx:pos[g])
            {
                ...
                //cout << idx <<" " <<ele+1<< endl;
                int cost = 0;
                if(ele+1==m-1&&idx!=n-1) ++cost;
                dp[idx][ele+1]=min(dp[idx][ele+1],dp[i][ele]+cost+(((idx-i)>1)?1:0));
            }
        }
   }
...
//    }
   ...
   for(int i=0;i<n;i++)
   {
        output = min(output,dp[i][m-1]);
   }

   cout << (output==1e9?-1:output) << "\n";
