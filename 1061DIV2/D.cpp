...
{   
    ...
    for(int i=0;i<=k;i++)
    {
        ...
        {
            if(skip2.count(num)) continue;
            int p = (1ll<<i)&num;
            sum += (p>0?1:0);
            if(p==0) num_zeroes.push_back(num);
            if(p!=0) num_ones.push_back(num);
        }

        for(int idx=1;idx<n;idx++)
        {
            if(skip.count(idx)) continue;
            int m = query(idx,(1ll<<i));
            j += m;
            if(m==0) zeroes.push_back(idx);
            if(m!=0) ones.push_back(idx);
        }
        if(j==sum)
        {
            for(auto &ele:ones)
