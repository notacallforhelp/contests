...
{   
   ...
   for(int i=1;i<3;i++)
   {
        lcm_abc = (temp[i]*lcm_abc)/__gcd(temp[i],lcm_abc);
   }

   //cout << lcm_abc << "\n";

   int lcm_ab = a*b/__gcd(a,b);
   int lcm_bc = b*c/__gcd(b,c);
   ...
   int times_abc = m/lcm_abc;
   int times_ab = m/lcm_ab - times_abc;
   int times_ac = m/lcm_ac - times_abc;
   int times_a = m/a - times_ab - times_ac - times_abc;

   int times_bc = m/lcm_bc - times_abc;
   int times_b = m/b - times_bc - times_ab - times_abc;

   int times_c = m/c - times_ac - times_bc - times_abc;

   output_a = times_a*6 + (times_ac+times_ab)*3 + times_abc*2;
   output_b = times_b*6 + (times_bc+times_ab)*3 + times_abc*2;
   output_c = times_c*6 + (times_ac+times_bc)*3 + times_abc*2;

   cout << output_a << " " << output_b << " " << output_c<< "\n";
