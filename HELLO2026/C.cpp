...
{   
   ...
   int smallerside = min(k-1,n-k);
   int biggerside = max(k-1,n-k);
   ...
   for(int i=1;i<=biggerside;i++)
   {
        if(2*i-1>m) break;
        int captured = i;
        int daysleft = m-(2*i-1);

        int low = 0;
        int high = smallerside;
        ...
        //cout << i << " " << captured << " " << extra << endl;

        ans = max(captured+extra,ans);
   }
