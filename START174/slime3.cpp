...
void solve()
{
    ...
    for(int i=1;i<n-1;i++)
    {
        ...
        if(ct_left>=2&&ct_right>=2)
        {
            output = max(output,A[i]);
        }
        else if(ct_left>=2)  //here ct_right is 1
        {
            output = max(output,A[i]-A[i+1]);
        }
        else if(ct_right>=2) //here ct_left is 1
        {
            output = max(output,A[i]-A[i-1]);
        }
        else //only possible if ct_left and ct_right both 1
        {
            int minimum = min(A[i-1],A[i+1]);
            int maximum = max(A[i-1],A[i+1]);

            int squeeze = abs(A[i]-minimum);

            output = max(output,max(squeeze,maximum)-min(squeeze,maximum));
        }
    }
    cout << output << endl;
}
