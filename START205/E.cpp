...
bool dfs(int vertex,int maintain,bool used,int receivedwater)
{
    int water_without_fix = k;
    int water_with_fix = k;
    if(vertex!=1)
    {
...
void solve()
{
    ...
    {
        cin>>leakiness[i];
        if(leakiness[i]==1&&!seenone)
        {
            seenone=true;
        }
        else if(leakiness[i]==1&&seenone)
        {
            ret = true;
        }
    }
