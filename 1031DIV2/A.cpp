...
void solve()
{
    ...
    if(x<=y)
    {
        int portion = max(0ll,ceil2(k-a+1,x));

        int left = k - x*portion;

        int p2 = max(0ll,ceil2(left-b+1,y));

        cout << portion+p2 << endl; return;
    }
    else
    {
        int portion = max(0ll,ceil2(k-b+1,y));

        int left = k - y*portion;

        int p2 = max(0ll,ceil2(left-a+1,x));
