...
void solve()
{
    ...
    pair<int,int> fr = prs[0];
    pair<int,int> sc = prs[1];

    if(1)
    {
        int diff = max(0ll,sc.first-(fr.first+a));
        int diff2 = 0;
        if(fr.second<sc.second)
        {
            diff2 = max(diff2,sc.second-(fr.second+b));
        }
        else
        {
            diff2 = max(diff2,fr.second-(sc.second+b));
        }
