...
pair<int,int> getpos(int n,int d,int x,int y)
{
    ...
    if(d<=block)
    {
        return getpos(n-1,d,x,y);
    }
    else if(d<=2*block)
    {
        return getpos(n-1,d-block,x+half,y+half);
    }
    else if(d<=3*block)
    {
        return getpos(n-1,d-2*block,x+half,y);
    }
    else if(d<=4*block)
    {
        return getpos(n-1,d-3*block,x,y+half);
    }
}
...
void solve()
{
    ...
    while(q--)
    {
        ...
        {
            int d; cin>>d;
            pair<int,int> op = getpos(n,d,1ll,1ll);
