...
void query(set<int> &T,int p,set<int> &discovered,vector<int> &A)
{
    int k = T.size()+1;
...
void query2(set<int> &discovered,int p,vector<int> &A)
{
    int k = discovered.size()+1;

    cout << "? "<< k << " ";
    for(auto &ele:discovered)
    {
        cout << ele << " ";
    }
    cout << p << endl;
...
void solve()
{
    ...
    for(int i=2;i<=2*n;i++)
    {
        query(T,i,discovered,A);
    }

    for(int i=1;i<=2*n;i++)
    {
        if(discovered.count(i)) continue;
        query2(discovered,i,A);
    }
    cout << "! ";
    for(int i=1;i<=2*n;i++)
    {
