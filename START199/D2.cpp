...
int dp(int i,int type)
{
    ...
    if(type==0)
    {
        ...
        ans = max(ans,1+dp(i+1,2));
    }

    if(type==1)
    {
        if(A[i-1]<A[i]) ans = max(ans,1+dp(i+1,1));
        if(A[i-1]<B[i]) ans = max(ans,1+dp(i+1,2));
    }

    if(type==2)
    {
        if(B[i-1]<A[i]) ans = max(ans,1+dp(i+1,1));
        if(B[i-1]<B[i]) ans = max(ans,1+dp(i+1,2));
    }
