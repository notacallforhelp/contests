...
void solve()
{
    ...
    if(n%2!=0)
    {
        cout << (n+1)/2 << endl; return;
    }

    int biggest_odd = ((n/2)%2!=0?n/2:(n/2-1));

    //int biggest_odd = max((n/2)-1,1ll);
    ...
    /*for(int i=n;i>=2;i-=2)
    {
        ...
        biggest_odd = max(biggest_odd,t);
    }*/

    //cout << biggest_odd << endl;

    int output = n/2 + (biggest_odd+1)/2 ;
    cout << output << endl;
}
