...
{   
   ...
   int a = lowestSetBit(s);
   int b = lowestSetBit(m);
   ...
   for(int i=61;i>=0;i--)
   {
        ...
        if((1ll<<i)&m)
        {
            powers_m.push_back(1);
        }
        else
        {
            powers_m.push_back(0);
        }
   }
   ...
   auto check = [&](int mid){
        ...
        for(int i=0;i<62;i++)
        {
            ...
            if(powers_m[i]==0&&temp[i]>0) return false;
        }
